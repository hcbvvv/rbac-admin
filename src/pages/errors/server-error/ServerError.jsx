...
const ServerError = () => {
  ...
  return (
    <div className={styles.serverErrorContainer}>
      <Card className={styles.serverErrorCard}>
        <Result
          status="500"
          title="500"
          subTitle="抱歉，服务器出现错误，请稍后再试。"
          className={styles.serverErrorResult}
          extra={
            <Space wrap className={styles.serverErrorExtra}>
              <Button 
                type="primary" 
                icon={<HomeOutlined />}
        ...
        <div className={styles.serverErrorInfo}>
          <Text className={styles.serverErrorPath}>
            当前访问路径：{location.pathname}
          </Text>
          <br />
          <Text className={styles.serverErrorId}>
        ...
        {(process.env.NODE_ENV === 'development' || errorInfo) && (
          <Collapse 
            ghost 
            className={styles.errorDetailsPanel}
          >
            <Panel 
              header={
                <Space className={styles.errorDetailsHeader}>
